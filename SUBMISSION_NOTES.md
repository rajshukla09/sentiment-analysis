...
- To demo 3 simultaneous jobs, open three browser tabs or run three `curl -F "file=@sample.pdf;type=application/pdf" <api>/jobs` commands quickly. Copy each returned job ID into the status page or open `/jobs/{id}` for each one; each should move independently from `queued` to `running` to `completed` or `failed`.
